...
'''
python tools/demo.py image -f exps/ppyoloe/ppyoloe_crn_l_voc2012.py -c PPYOLOE_outputs/ppyoloe_crn_l_billiards/best_ckpt.pth --path assets/new.jpg --conf 0.45 --tsize 1280 --save_result --save_txt --device gpu
'''
...
print('1:',sys.path)
from mmdet.utils.boxes import xyxy2xywh, xyxy2xywh2
...
def make_parser():
    ...
    parser.add_argument(
        "--save_txt",
        action="store_true",
        help="whether to save the inference txt result of image/video",
    )
...
class PPYOLOEPredictor(object):
    ...
    def inference(self, img):
        ...
        # gn = img.shape
        # gn = list(gn)[:2] * 2
        gn = torch.tensor(img.shape)[:2][[1,0,1,0]] # tensor([1920, 1080, 1920, 1080])
    ...
    def save_labels(self, output, gn, save_txt_path,img_name):
        print('gn:',gn)
        output = output[0].cpu()
        bboxes = output #xyxy
        for cls, conf, *xyxy in reversed(bboxes):
            # xywh = xyxy2xywh2(torch.tensor(xyxy).view(1,4))
            # print("00:",xywh)
            # print('11:',torch.tensor(xywh).view(1,4))
            # print('22:',xyxy2xywh2(torch.tensor(xywh).view(1,4))/gn)
            xywh = (xyxy2xywh2(torch.tensor(xyxy).view(1,4))/gn).view(-1).tolist()
            line = (cls, *xywh)
            txt_path = os.path.join(save_txt_path, img_name[:-4]) 
...
def image_demo(predictor, vis_folder, path, current_time, save_result, save_txt):
    ...
    for image_name in files:
        outputs, img_info, gn = predictor.inference(image_name)
        ...
        if save_txt:
            ...
            save_txt_path = os.path.join(save_folder,'labels')
            os.makedirs(save_txt_path, exist_ok=True)
            logger.info("Saving detection txt result in {}".format(save_txt_path))
            line = predictor.save_labels(outputs, gn, save_txt_path, img_info["file_name"])
            # for cls, conf, *xyxy in reversed(outputs[0]):
            #     xywh = xyxy2xywh2(torch.tensor(xyxy).view(1,4))
            print('line:',line)
            # line = (cls, *xywh)
            # with open(save_txt_path + '.txt','a') as f:
            #     f.write(('%g' * len(line)).rstrip() % line + '\n')
